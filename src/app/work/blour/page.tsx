...
export default function BlourPage() {
  ...
  return (
    <main className="relative mx-auto w-full max-w-[90rem] pb-6">
      ...
      {/* Hero */}
      <section className="relative mx-3 mt-3 px-6 pb-7 pt-5 sm:mx-6 sm:mt-6 sm:px-10 sm:pb-8 sm:pt-6 lg:px-14">
        <SiteNav flush />
        <div className="relative z-10 mt-8 flex flex-col items-center sm:mt-10">
          <LogoTitle />
      ...
      {/* Intro */}
      <section className="relative mx-3 px-6 pb-12 pt-2 sm:mx-6 sm:px-10 sm:pb-10 sm:pt-4 lg:px-14">
        ...
        <section key={title.en} className="relative mx-3 px-6 py-10 sm:mx-6 sm:px-10 sm:py-16 lg:px-14 lg:py-20">
          ...
          >
            ...
            <ul className={`space-y-5 ${idx % 2 === 1 ? "lg:order-1" : ""}`}>
              {points.map((p) => (
                <li key={p.en} className="flex gap-3 text-base leading-relaxed sm:text-lg text-ink">
